...
import styles from '../app/tiktok-downloader/page.module.css'
...
export default function TikTokDownloaderClient() {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await apiClient.post('/api/scrape-tiktok', {
        url: url.trim(),
      })

      setResult(response.data)
      if (!response.data?.count || response.data.count === 0) {
        setError('No video found for this TikTok.')
  ...
  const handleDownloadVideo = async () => {
    ...
    try {
      setLoading(true)
      setWarning(null)
      const response = await apiClient.post('/api/download-tiktok', {
        url: url.trim(),
  ...
  return (
    <>
      <section className={styles.hero}>
        <h1 className={styles.mainTitle}>TikTok Video Downloader</h1>
        <p className={styles.subtitle}>
          Download TikTok Videos Instantly - Fast, Free, and High Quality
        </p>
          ...
          <div className={styles.resultsContainer}>
            ...
            <div className={styles.singleImageWrapper}>
              <img
                ...
                alt={result.media[0].title || 'TikTok Video'}
                className={styles.singleImage}
              />

              <div className={styles.videoInfo}>
                <h3 className={styles.videoTitle}>{result.media[0].title || 'TikTok Video'}</h3>
